...
class accountMgr(object):
    ...
    def save_token(self,account,symbol,quantity,precision,contract):
       ...
       try:
            ...
            sql = "INSERT INTO tokens (account,symbol ,balance,symbol_precision,contract_owner)  VALUES('%s','%s' ,'%s' , %d ,'%s') ON DUPLICATE KEY UPDATE balance='%s',symbol_precision=%d" %(account,symbol,quantity,precision,contract,quantity,precision)
